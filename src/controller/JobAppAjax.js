...
const getAjaxForm = async (req, res) => {
    res.render('formAjax')
}
...
const postJobFormAjax = async (req, res) => {
    ...
    con.query(selectQ, (err, result) => {
        if (err) throw err;
        res.render('listEmpAjax', { result })
    })
}

const getEmpAjax = async (req, res) => {
    ...
    res.render('updateFormAjax', { result, result2, result3, result4, result5, result6, result7, php, mysql, laravel, oracle })
}

const updateFormAjaxPost = async (req, res) => {
    var emp_id = req.params.emp_id;
